...
void render_bg_line_text(PPU* ppu, int bg) {
    ...
    if (ppu->master->io.bgcnt[bg].palmode) {
        ...
        dword row =
            ppu->master->vram.w[((tile_addr + 8 * tmpfy) % 0x10000) >> 2];
        row |= (dword) ppu->master->vram
                   .w[((tile_addr + 8 * tmpfy + 4) % 0x10000) >> 2]
               << 32;
        ...
        if (tile.vflip) tmpfy = 7 - fy;
        word row =
            ppu->master->vram.w[((tile_addr + 4 * tmpfy) % 0x10000) >> 2];
        for (int x = 0; x < GBA_SCREEN_W; x++) {
            byte col_ind;
            if (tile.hflip) {
...
void render_obj_line(PPU* ppu, int i) {
    ...
    if (o.disable_double & !o.aff) return;
    byte w, h;
    switch (o.shape) {
        case OBJ_SHAPE_SQR:
    ...
    byte ty = yofs >> 3;
    byte fy = yofs & 0b111;
    word tile_addr = o.tilenum * 32;

    if (ppu->master->io.dispcnt.bg_mode > 2 && tile_addr < 0x4000) return;
    ...
    if (o.palmode) {
        tile_addr +=
            ty * 64 * (ppu->master->io.dispcnt.obj_mapmode ? w / 8 : 16);
        if (o.hflip) tile_addr += 64 * (w / 8 - 1);
        dword row =
            ppu->master->vram.w[(0x10000 + (tile_addr + 8 * fy) % 0x8000) >> 2];
        row |= (dword) ppu->master->vram
        ...
        for (int x = 0; x < w; x++) {
            ...
            if (sx < GBA_SCREEN_W) {
                ...
                if (o.mode == OBJ_MODE_OBJWIN &&
                    (ppu->master->io.dispcnt.win_enable & (1 << OBJWIN))) {
                    if (col_ind) {
                        ppu->window[x] = OBJWIN;
                    }
            ...
            if (fx == 8) {
                ...
                row = ppu->master->vram
                          .w[(0x10000 + (tile_addr + 8 * fy) % 0x8000) >> 2];
                row |=
                    (dword) ppu->master->vram
                        .w[(0x10000 + (tile_addr + 8 * fy + 4) % 0x8000) >> 2]
                    << 32;
            }
        }
        ...
        tile_addr +=
            ty * 32 * (ppu->master->io.dispcnt.obj_mapmode ? w / 8 : 32);
        if (o.hflip) tile_addr += 32 * (w / 8 - 1);
        word row =
            ppu->master->vram.w[(0x10000 + (tile_addr + 4 * fy) % 0x8000) >> 2];
        for (int x = 0; x < w; x++) {
            int sx = (o.x + x) % 512;
            ...
            if (fx == 8) {
                ...
                row = ppu->master->vram
                          .w[(0x10000 + (tile_addr + 4 * fy) % 0x8000) >> 2];
            }
        }
    }
}
...
void compose_lines(PPU* ppu) {
    ...
    for (int x = 0; x < GBA_SCREEN_W; x++) {
        byte layers[6];
        int l = 0;
        bool put_obj = ppu->draw_obj && !(ppu->objline[x] & (1 << 15));
        for (int i = 0; i < 4 && l < 2; i++) {
            ...
            if (ppu->draw_bg[sorted_bgs[i]] &&
                !(ppu->bgline[sorted_bgs[i]][x] & (1 << 15))) {
        ...
        hword color0;
        if (layers[0] < 4) color0 = ppu->bgline[layers[0]][x];
        else if (layers[0] == LOBJ) color0 = ppu->objline[x];
        else color0 = ppu->master->pram.h[0];
...
void draw_line(PPU* ppu) {
    ...
    render_bgs(ppu);
    render_objs(ppu);
    if (ppu->master->io.dispcnt.win_enable) render_windows(ppu);
